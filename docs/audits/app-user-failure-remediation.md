...
| 3 | **Four `createTenantClient` callers remain, and they are LATENT, not safe** — `dashboard.ts` (2), `tenant-notification-settings.ts` (3), `base.repository.ts` (1). A `createTenantClient` client carries the Prisma filter and no GUC, so under the tripwire it raises the moment no earlier statement has left a context on the `max: 1` pool. | Each becomes `getTenantPrisma()` (they have sessions) — three files, six call sites. Frozen by the guard in §7 so adding a seventh is a reviewable edit. |
| 4 | **`(owner)/actions/notifications.ts`'s two sites were LATENT and are now fixed, but `/dashboard` still passes for a reason nobody should rely on.** quick-604 measured it `pass`; the likely reason is the `max: 1` pool inheriting a GUC an earlier scoped statement set. | Nothing here — recorded so the same reasoning is not used to declare another surface safe. |
...
| 8 | **`npm run audit:rls-policy-drift` reads PRODUCTION regardless of `DATABASE_URL`.** It imports `scripts/_bootstrap-env`, which does `DATABASE_URL = DIRECT_URL` unconditionally, and `DIRECT_URL` is production in every env file. Passing `DATABASE_URL=<staging>` alone is silently ignored. | `DIRECT_URL` must be exported too — which is what §9's staging run does. Longer term the script should refuse when `DATABASE_URL` was set explicitly and then overwritten. It is read-only, so nothing was written; but an operator would believe they had measured staging. |
