 ...
 *
 * Triggers a Samsara GPS location sync for a tenant.
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Look up enabled Samsara integration for this tenant (bypass RLS)
    ...
    if (!integration) {
      return NextResponse.json(
        { error: 'Samsara integration not enabled' },
        { status: 404 }
      );
    }
    ...
    if (!apiToken) {
      return NextResponse.json(
        { error: 'Samsara API token not configured' },
        { status: 400 }
      );
    }

    // Run sync
    const result = await syncSamsaraLocations(tenantId, apiToken);
