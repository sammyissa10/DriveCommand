...
const HOOK_PATH = resolve(__dirname, '620-tc001-driver-hook.js').split('\\').join('/');
...
{
  const l = '[server-target] NODE_OPTIONS --require 620-tc001-driver-hook.js (fixed dual-form TC001 detector)';
  writeSync(fd, l + '\n');
  console.error(l);
}
