/**
 * quick-620 — THE REPO-WIDE `app.bypass_rls` REMAINDER, MEASURED (copy of 619's; target, arithmetic and
 * counter-assertion changed, plus an other-surfaces-unchanged check in --after mode).
 *
 *   npx tsx scripts/audit/620-repo-remainder.ts [--after]
 *
...
function main() {
  ...
  // quick-620: 619's floor of 50 sits ABOVE this task's correct answer (45). A floor is anti-vacuity
  // (a walker returning nothing), not the expectation — lowered to 40, still far above zero.
  checks.push({
    name: 'FLOOR — total statements >= 40',
    pass: shipping.length >= 40,
    detail: `${shipping.length} >= 40`,
  });
  ...
  // quick-620: before, the target is a file quick-619 routed (still zero); after, it is a file THIS task routed.
  const AFTER = process.argv.includes('--after');
  const cPath = AFTER ? 'src/app/api/v1/messages/thread/route.ts' : 'src/lib/geofencing/geofence-check.ts';
  ...
  const ROUTED_BY_620 = Number(process.env.ROUTED_BY_620 ?? '14');
  const EXPECTED = AFTER ? 59 - ROUTED_BY_620 : 59;
  if (AFTER) {
    const before = JSON.parse(readFileSync(resolve(EVIDENCE_DIR, '01-repo-remainder-before.json'), 'utf8'));
    const key = (x: { file: string; line: number }) => `${x.file}:${x.line}`;
    const b = before.statements.filter((x: { surface: string }) => x.surface !== 'API_V1').map(key).sort();
    const a = shipping.filter((x) => x.surface !== 'API_V1').map(key).sort();
    checks.push({
      name: 'OTHER SURFACES UNCHANGED — every non-API_V1 statement identical file:line before and after',
      pass: JSON.stringify(a) === JSON.stringify(b),
      detail: `before ${b.length} / after ${a.length}`,
    });
  }
  checks.push({
    name: AFTER ? `ARITHMETIC — 59 at quick-620 start − ${ROUTED_BY_620} routed from API_V1 = ${EXPECTED}` : 'ARITHMETIC — 59 at quick-620 start (quick-619 close)',
  ...
  const out = {
    ...
    arithmetic: {
      ...
      routedBy616: 2,
      liveAt617Start: 175,
      routedBy617: 67, liveAt618Start: 108, routedBy618: 16, liveAt619Start: 92, routedBy619: 33, liveAt620Start: 59, routedBy620: AFTER ? ROUTED_BY_620 : 0,
  ...
  writeFileSync(resolve(EVIDENCE_DIR, process.argv.includes('--after') ? '05-repo-remainder-after.json' : '01-repo-remainder-before.json'), JSON.stringify(out, null, 2) + '\n');
