/**
 * quick-624 — COPY of 620-start-staging-server.js; the ONLY changes: it preloads 624-guc-trace-hook.js (which
 * requires 620-tc001-driver-hook.js first) and sets Q624_TRACE=on. 620's text follows.
 *
...
const HOOK_PATH = resolve(__dirname, '624-guc-trace-hook.js').split('\\').join('/');
...
const env = {
  ...
  // quick-624: switch the tracing hook on (it requires 620's detector itself).
  Q624_TRACE: 'on',
  CRON_SECRET,
...
{
  const l = '[server-target] NODE_OPTIONS --require 624-guc-trace-hook.js (loads 620-tc001-driver-hook.js, then the connection tracer)';
  writeSync(fd, l + '\n');
  console.error(l);
}
