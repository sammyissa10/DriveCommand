/**
 * quick-627 — step 5. A COPY of 626-cache-hit-rate.ts, so the adopted stack is measured on exactly the traffic quick-626
 * used: request shapes, tenants, warm-up, instrument and percentile code are byte-identical. Changed: the evidence
 * directory, a `label` on each result, and the refusal prefix. `shipped` and `adopted` both import src; which code they
 * measure is decided by what is on disk when the process starts (the three quick-627 files stashed = shipped).
 *
 *   npx tsx scripts/audit/627-latency.ts --run --stacks adopted|shipped --blind-read --label <text> --out <file.json>
 *
 * ─── quick-626 header, unchanged ───
...
if (!raw.includes(STAGING_REF) || raw.includes(PRODUCTION_REF)) {
  console.error('627-latency: REFUSING — STAGING_DATABASE_URL_APP_USER does not name staging');
  process.exit(1);
}
...
async function main() {
  ...
  for (const c of [1, 4]) for (const k of stacks) {
    ...
    r.label = a.includes('--label') ? a[a.indexOf('--label') + 1] : k;
    results.push(r);
