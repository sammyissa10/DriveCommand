/**
 * quick-619 — start `next dev` pointed at STAGING as `app_user`, tripwire armed.
 *
 *   node scripts/audit/619-start-staging-server.js <absolute-log-path>
 *
...
if (!logPath) {
  console.error('usage: 619-start-staging-server.js <absolute-log-path>');
  process.exit(1);
}
...
function refuse(msg) {
  console.error(`619-start-staging-server: REFUSING — ${msg}`);
  process.exit(1);
}
