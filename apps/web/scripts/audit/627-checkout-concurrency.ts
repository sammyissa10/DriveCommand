/**
 * quick-627 — step 1 (copy of 625-checkout-concurrency.ts). Stacks: `cand` = 627-checkout-candidate.ts (leave + cache
 * + unbatch), `real` = whatever src/lib/db/prisma.ts + getTenantPrismaForOrg are on disk (shipped before step 4,
 * adopted after). Evidence path and stack set changed; iteration, judging and waves are 625's verbatim.
 *
...
if (!raw.includes(STAGING_REF) || raw.includes(PRODUCTION_REF)) {
  console.error('627-checkout-concurrency: REFUSING — STAGING_DATABASE_URL_APP_USER does not name staging');
  process.exit(1);
}
...
async function buildStack(kind: Stack, max: number) {
  ...
  const pool = p.createCandidatePool(
    ...
    { noContext: 'leave', armTripwire: true },
  );
  const base = p.createCandidateBaseClient(pool);
  return { tenant: async (t: string) => p.createCandidateTenantClient(base, t) as Db, bare: base as Db, pool };
}
...
async function main() {
  ...
  if (a.includes('--bench')) {
    const results = (['real', 'cand'] as Stack[]).map((k) => spawn(['--bench-one', k], '__BENCH__'));
  ...
  const stacks = (a.includes('--stacks') ? a[a.indexOf('--stacks') + 1].split(',') : ['cand', 'real']) as Stack[];
  const label = a.includes('--label') ? a[a.indexOf('--label') + 1] : '';
  const outName = a.includes('--out') ? a[a.indexOf('--out') + 1] : '01-checkout-concurrency.json';
  const plan: [Stack, number, boolean][] = [];
  for (const errors of [false, true]) for (const kind of stacks) for (const max of [1, 5]) plan.push([kind, max, errors]);
  const results = [];
  for (const [kind, max, errors] of plan) {
    if (only && kind !== only) continue;
    const r = spawn(['--config', kind, '--max', String(max), ...(errors ? ['--errors'] : [])], '__CONFIG__');
    r.label = r.stack === 'real' ? label || 'real (on-disk src)' : 'candidate leave+cache+unbatch';
    results.push(r);
    console.log(JSON.stringify(r));
  }
  if (!only) writeFileSync(resolve(EVIDENCE, outName), JSON.stringify({ generated: new Date().toISOString(), results }, null, 2) + '\n');
}
