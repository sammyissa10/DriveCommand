/**
 * quick-617 — THE REPO-WIDE `app.bypass_rls` REMAINDER, MEASURED.
 *
 *   npx tsx scripts/audit/617-repo-remainder.ts
 *
 * WHY NOT JUST RE-RUN `616-bypass-census.ts`
 * ------------------------------------------
 * It writes `01-census.json` into **quick-616's** evidence directory. Re-running
 * it would overwrite a closed task's artefact — byte-for-byte the destruction
 * quick-610 documented for `604-click-through.ts`'s passes 1 and 2. This script
 * uses the SAME AST method over the SAME corpus and writes into quick-617's own
 * evidence instead.
 *
...
function main() {
  ...
  const cPath = 'src/app/api/mobile/driver/incidents/route.ts';
  ...
  const EXPECTED = 175 - 67;
  checks.push({
    name: 'ARITHMETIC — 177 census − 2 (quick-616) − 67 (quick-617) = 108',
  ...
  writeFileSync(resolve(EVIDENCE_DIR, '06-repo-remainder.json'), JSON.stringify(out, null, 2) + '\n');
