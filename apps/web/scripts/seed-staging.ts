  ...
  if (template) {
    ...
    template = await prisma.routeTemplate.create({
      data: {
        ...
        /**
         * quick-606 — THE FIXTURE WAS INCOHERENT, and it cost quick-604 a whole
         * surface.
         *
         * `autoGenerateDaysAhead` defaults to **7** (`schema.prisma:2297`,
         * confirmed as the column default on staging), so a template created
         * without naming it is auto-generating. This one is `scheduleType:
         * 'on_call'` with no `recurrenceRule`, and `generateDispatches` throws
         * `Template has no recurrenceRule — nothing to generate` on exactly that
         * combination. `/api/cron/carrier-auto-dispatch` therefore 500'd on the
         * fixture and its tenant-scoped database work has NEVER been measured
         * (quick-604 §8 item 10 — "a NOT_INVOKED in all but name").
         *
         * `0` rather than a recurrence rule, deliberately: an on-call template
         * that does not auto-generate is a COHERENT fixture, whereas a template
         * carrying a rule would still fail one step deeper —
         * `dispatch-generator.ts:335` dereferences `template.defaultDriverId!`
         * and `defaultTruckId!`, both NULL on every seeded template. Exercising
         * the generation path needs three more fixture fields and a cleanup path
         * for the trips, loads and stops it writes; that is named in
         * `docs/audits/app-user-failure-remediation.md` §8, not smuggled in here.
         */
