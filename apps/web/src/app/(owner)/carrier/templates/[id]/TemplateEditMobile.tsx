...
export function TemplateEditMobile({
  ...
  async function save() {
    ...
    try {
      const result = await saveRouteTemplate({
        ...
        // Stops are not edited here yet — pass them through untouched so the
        // server's "at least one stop" rule still passes and none are dropped.
  ...
  const stopCount = initialData.stops.length;
  ...
  return (
    <MobileScreen className="pb-10 pt-2">
      ...
      <div className="space-y-6">
        ...
        {/* Stops are carried through untouched until the ds stop builder lands */}
        <div>
          <SectionHeader title="Stops" />
          <div className="flex items-center gap-3 rounded-[20px] bg-ds-card p-4">
            <MapPin className="h-5 w-5 shrink-0 text-ds-txt3" />
            <div className="min-w-0 flex-1">
              <p className="text-[15px] text-ds-txt">
                {stopCount} stop{stopCount === 1 ? '' : 's'} on this route
              </p>
              <p className="mt-0.5 text-[13px] text-ds-txt3">
                Editing a template&apos;s stops is on desktop for now — saving here keeps them as they are.
              </p>
            </div>
          </div>
        </div>
