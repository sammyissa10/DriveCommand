...
export function LoadMessagesSection({ loadId, driverId, initialMessages }: LoadMessagesSectionProps) {
  const [state, formAction, isPending] = useActionState(sendOwnerLoadReply, null);
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-lg font-semibold text-card-foreground">Load Messages</h2>
        {messages.length > 0 && (
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Drivers can send messages from their mobile app.
            </p>
          </div>
        ) : (
          <>
      ...
      {/* No driver assigned notice */}
      {!driverId && (
        <p className="mb-4 text-xs text-muted-foreground border border-border rounded-lg px-3 py-2 bg-muted/30">
          No driver assigned — dispatch this load to enable messaging.
        </p>
      )}
