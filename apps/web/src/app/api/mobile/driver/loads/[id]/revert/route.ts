...
export async function PATCH(
  ...
  try {
     ...
     * audit-columns extension to start writing createdById/updatedById, a
     * behaviour change a routing fix must not make (quick-610).
     *
     * quick-617 STOPPED this file. The findUnique below carries a top-level
     * select that omits tenantId, and the old post-check read that as
     * `undefined !== tenantId` and discarded the row FOR ITS OWN TENANT —
     * which, with the `if (!load) return 404` three lines down, would have made
     * this route answer "not found" on every request. quick-618 moved the
     * tenant predicate into the findUnique `where`, so the select no longer
     * decides isolation and is left byte-identical here.
     */
