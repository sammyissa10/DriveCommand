/**
 * GET /api/driver-pay/reports/load-profitability
 *
 * Returns per-load revenue minus driver pay, paginated + sortable.
 * Supports ?format=csv for streaming CSV download (uses large pageSize for CSV).
...
export async function GET(req: NextRequest) {
  try {
    ...
    // For CSV, use a very large page size to get all rows in one query.
    // NOTE: For very large datasets (>100k loads), a cursor-based approach
    // should be used in Phase 11. For now this is acceptable.
    ...
    const result = await computeLoadProfitability(prisma, session.tenantId, range, {
    ...
    if (format === 'csv') {
      const headers = ['Load Ref', 'Client', 'Revenue', 'Driver Pay', 'Profit', 'Margin %', 'Driver Count', 'Created'];
      const rows = result.rows.map((r) => [
        ...
        r.totalDriverPay,
        r.profit,
        r.profitMarginPct != null ? r.profitMarginPct.toFixed(1) : '',
        String(r.driverCount),
        r.createdAt,
      ]);
      const start = range.start.toISOString().slice(0, 10);
      const end = range.end.toISOString().slice(0, 10);
      return streamCsvResponse(headers, rows, `load-profitability-${start}-${end}.csv`);
    }

    return NextResponse.json(result);
  } catch (err) {
    logger.error('reports/load-profitability error', err);
