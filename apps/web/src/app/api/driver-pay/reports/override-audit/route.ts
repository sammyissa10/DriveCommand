/**
 * GET /api/driver-pay/reports/override-audit
 *
 * Returns LoadDriverAssignment rows with overrideReason set, with dispatcher summary.
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await computeOverrideAudit(prisma, session.tenantId, range, {
    ...
    if (format === 'csv') {
      const headers = [
        'Load Ref', 'Driver', 'Override Reason', 'Pay Type',
        'Base Rate', 'Rate Unit', 'Created By (UUID)', 'Created At',
      ];
      ...
      return streamCsvResponse(headers, rows, `override-audit-${start}-${end}.csv`);
    }

    return NextResponse.json(result);
  } catch (err) {
    logger.error('reports/override-audit error', err);
