/**
 * GET /api/driver-pay/reports/settlement-history
 *
 * Returns full all-status settlement history, paginated + sortable, with anomaly flag.
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await computeSettlementHistory(
      prisma,
      session.tenantId,
      range,
      filters,
    ...
    if (format === 'csv') {
      const headers = [
        'Settlement ID', 'Driver', 'Period Start', 'Period End', 'Status',
        'Gross Taxable', 'Gross Non-Taxable', 'Total Deductions', 'Net Pay', 'Paid At', 'Anomaly?',
      ];
      ...
      return streamCsvResponse(headers, rows, `settlement-history-${start}-${end}.csv`);
    }

    return NextResponse.json(result);
  } catch (err) {
    logger.error('reports/settlement-history error', err);
