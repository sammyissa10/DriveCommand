...
export function DispatchMessages({ dispatchId, driverUserId }: DispatchMessagesProps) {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      if (driverUserId) params.set('driverId', driverUserId);
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch('/api/v1/messages/send', {
        ...
        body: JSON.stringify({ recipientId: driverUserId, body: body.trim(), dispatchId }),
      });
  ...
  const handleVoiceSend = async (audioBlob: Blob) => {
    ...
    const sendRes = await fetch('/api/v1/messages/send', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ recipientId: driverUserId, body: 'Voice message', dispatchId, audioUrl }),
    });
    ...
    <div className="rounded-lg border border-border bg-card overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-foreground">Dispatch Messages</h3>
        </div>
        <Link
      ...
      <div className="h-[300px] overflow-y-auto p-4 flex flex-col gap-2">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-2">
            ...
            <p className="text-sm text-muted-foreground">No messages for this dispatch yet</p>
          </div>
        ) : (
          <>
            {messages.map((msg) => (
