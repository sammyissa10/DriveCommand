...
import { Bell, Truck, MessageSquare } from 'lucide-react';
...
/** Driver-side deep-link resolution */
function resolveDriverLink(entityType: string): string {
  switch (entityType) {
    case 'dispatch':
      return '/my-route';
    case 'message':
...
export function DriverNotificationPanel({ onClose, onMarkedAllRead }: DriverNotificationPanelProps) {
  ...
  async function fetchNotifications() {
    try {
      const res = await fetch('/api/driver/notifications?limit=20');
      if (!res.ok) return;
  ...
  async function handleMarkAllRead() {
    try {
      await fetch('/api/driver/notifications/mark-read', {
        method: 'PATCH',
  ...
  async function handleNotificationClick(notification: DriverNotification) {
    try {
      await fetch('/api/driver/notifications/mark-read', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
    ...
    const link = resolveDriverLink(notification.entityType);
    router.push(link);
    onClose();
  }

  return (
    <div className="w-[340px] max-h-[480px] overflow-y-auto bg-popover border rounded-lg shadow-lg z-50 flex flex-col">
      {/* Sticky header */}
          ...
          <>
            ...
            {notifications.length >= 20 && (
              <div className="px-4 py-3 text-center text-xs text-muted-foreground border-t">
                Showing most recent notifications
              </div>
            )}
          </>
        )}
