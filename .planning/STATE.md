...
Last activity: 2026-07-21 - Completed quick task 476: Route creation Origin/Destination/Stop address fields converted to a native facility dropdown (lists CarrierFacility records) with an "Enter manually…" fallback that reveals the existing address autocomplete. Shared FacilityAddressSelect component reused by desktop RouteForm and mobile RouteCreateMobile; facility select fills the address string + lat/lng (with geocode fallback) so OSRM distance still works; createRoute/updateRoute FormData contract untouched (tkt-0078,0079).
