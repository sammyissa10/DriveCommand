# quick-612 — AutomationRule policy matrix (AFTER)
...
- **[ALL] bypass_rls_policy** (PERMISSIVE)
  ...
  - WITH CHECK : `(none)`
- **[DELETE] automation_rule_delete_policy** (PERMISSIVE)
  - USING      : `("tenantId" = current_tenant_id())`
  - WITH CHECK : `(none)`
- **[INSERT] automation_rule_insert_policy** (PERMISSIVE)
  - USING      : `(none)`
  - WITH CHECK : `("tenantId" = current_tenant_id())`
- **[SELECT] tenant_isolation_policy** (PERMISSIVE)
  - USING      : `((scope = 'SYSTEM'::"AutomationScope") OR ("tenantId" = current_tenant_id()))`
  - WITH CHECK : `(none)`
- **[UPDATE] automation_rule_update_policy** (PERMISSIVE)
  - USING      : `("tenantId" = current_tenant_id())`
  - WITH CHECK : `("tenantId" = current_tenant_id())`
...
| U2 | UPDATE | must be REFUSED | UPDATE a SYSTEM rule (touch a non-tenantId column) | 0 row(s) affected | SYSTEM rows still present = **6** |
| U3 | UPDATE | must be REFUSED | CAPTURE: UPDATE SYSTEM rules SET tenantId = own | 0 row(s) affected | SYSTEM rows STILL scope=SYSTEM with tenantId NULL = **6** |
...
| D2 | DELETE | must be REFUSED | DELETE the 6 SYSTEM platform rules | 0 row(s) affected | SYSTEM rows still present = **6** |
