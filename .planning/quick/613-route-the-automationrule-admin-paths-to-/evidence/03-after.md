# quick-613 — routed `AutomationRule` sites, both directions (AFTER)
...
grants on `"AutomationRule"`: `app_admin: SELECT` · `app_admin: UPDATE` · `app_user: DELETE` · `app_user: INSERT` · `app_user: SELECT` · `app_user: UPDATE`
...
|---|---|---|---|---|---|---|
| 1 | ADMIN — app_admin, no GUC | must SUCCEED | every rule, and a run count spanning BOTH tenants | rules_visible = **8** · runs_counted = **2** | — | — |
...
| 2 | ADMIN — app_admin, no GUC | must SUCCEED | both runs and both tenant names | runs_visible = **2** · tenant_names_visible = **2** | — | — |
...
| 3 | ADMIN — app_admin, no GUC | must SUCCEED — the grant is live | flip isActive on a platform rule | 1 row(s) affected | the SYSTEM rule still carries its original isActive = **1** | — |
...
| 5a | ADMIN — app_admin, no GUC | must SUCCEED | reads the platform rule | n = **1** | — | — |
...
| 5b | ADMIN — app_admin, no GUC | must SUCCEED | reads tenant B's TENANT-scoped rule | n = **1** | — | — |
